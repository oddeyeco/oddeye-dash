                                            ...
                                            <c:when test="${config.getValue().path == 'alowswitch'}">
                                                <sec:authorize access="hasRole('ROLE_CAN_SWICH')">
                                                    <c:if test="${model[config.getValue().path]}">
                                                        <a href="<c:url value="/impersonate/login?username=${model.getEmail()}"/>" class="btn btn-info btn-xs" value="${model.getId()}"><i class="fa fas fa-pencil-alt"></i> <spring:message code="adminlist.switch"/> </a>    
                                                    </c:if>
                                                </sec:authorize>
                                            </c:when>
                                            <c:otherwise>
